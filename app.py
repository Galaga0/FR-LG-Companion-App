...
st.set_page_config(page_title="Firered/Leafgreen App", layout="wide")
...
def migrate_state(state: Dict) -> Dict:
    ...
    stg.setdefault("default_level", 20)
...
def build_kanto_state_from_web_cached() -> Dict:
    ...
    return {
        ...
        "stones": {k: 0 for k in STONE_ITEMS},
        "settings": {
            "unique_sig": True, "default_level": 20, "hide_spinner": True,
...
def render_pokedex():
    ...
    default_level = int(STATE["settings"].get("default_level", 20))
    ...
    with st.expander("Add Pokémon to Pokédex", expanded=False):
        ...
        level = col[1].number_input("Level", 1, 100, int(STATE["settings"].get("default_level", 20)), 1, key="add_level")
...
def render_team():
    ...
    with col_sync:
        with st.expander("Sync levels (ALL roster)", expanded=True):
                    with st.form("sync_levels_form", clear_on_submit=False):
                        col = st.columns([1,1])
                        target_level = col[0].number_input("Set roster level to", 1, 100, int(STATE["settings"].get("default_level", 20)), 1, key="sync_target_level")
    ...
    with col_add:
        ...
        with st.expander("Add Pokémon to Pokédex", expanded=False):
                    ...
                    species_name = label_to_name.get(choice_label, None)
                    level = col[1].number_input("Level", 1, 100, int(STATE["settings"].get("default_level", 20)), 1, key="add_level")
                    auto_moves = col[2].checkbox("Auto-fill moves from learnset", value=True, key="add_automoves")
...
def render_matchup():
    ...
    with st.form("sheet_pick_form", clear_on_submit=False):
        ...
        mon_labels = [f"{i+1}. Row {m.get('source_row','?')} — {m['species']} Lv{m['level']} (Total {m.get('total',0)})" for i, m in enumerate(enc["mons"])]
    ...
    opp_label = enc["label"] + " — " + (f"{selected_mon_idx+1}. Row {opmon.get('source_row','?')} — {opmon['species']} Lv{opmon['level']} (Total {opmon.get('total',0)})")
    ...
    if b1.button("✅ Beat Pokémon (remove just this one)"):
        try:
            if len(enc["mons"]) == 1:
                ...
                count = len(enc["mons"])
                STATE["opponents"]["cleared"].append({"what": "trainer","trainer": label_before,"count": count})
    ...
    if b2.button("🧹 Beat Trainer (remove entire encounter)"):
        try:
            ...
            STATE["opponents"]["cleared"].append({"what": "trainer","trainer": label_before,"count": len(enc["mons"])})
            STATE["opponents"]["encounters"].pop(selected_enc_idx)
            save_state(STATE)
            st.session_state['cached_sheet_pick'] = None
            do_rerun()
        ...
        if not log:
            ...
            for item in list(reversed(log[-15:])):
                if item.get("what") == "pokemon":
                    st.write(f"• Beat Pokémon: {item.get('species')} (Lv{item.get('level')}) — Trainer: {item.get('trainer')} (Row {item.get('row')})")
                else:
...
def render_evo_watch():
    ...
    rebuild_moves_default = c2.checkbox("Rebuild moves on evolve", value=False, key="evo_rebuild_moves_default")
...
def render_settings():
    ...
    with st.form("settings_form", clear_on_submit=False):
        uniq = st.checkbox("Avoid duplicate exact type combinations in team builder", value=STATE["settings"].get("unique_sig", True))
        default_level = st.number_input("Default level for adding new Pokémon", 1, 100, int(STATE["settings"].get("default_level", 20)), 1)
        hide_spinner = st.checkbox("Hide rerun spinner overlay", value=STATE["settings"].get("hide_spinner", True))
        st.markdown("---")
        st.subheader("Page visibility")
        vis = STATE["settings"].get("visible_pages", {})
        c1, c2, c3, c4 = st.columns(4)
        v_pokedex   = c1.checkbox("Pokédex", value=vis.get("pokedex", True))
        v_team      = c2.checkbox("Team", value=vis.get("team", True))
        v_matchup   = c3.checkbox("Matchup", value=vis.get("matchup", True))
        v_saveload  = c4.checkbox("Save/Load", value=vis.get("saveload", True))
        c5, c6, c7, c8 = st.columns(4)
        v_evo       = c5.checkbox("Evolution Watch", value=vis.get("evo", True))
        v_opponents = c6.checkbox("Opponents", value=vis.get("opponents", False))
        v_moves     = c7.checkbox("Moves DB", value=vis.get("moves", False))
        v_species   = c8.checkbox("Species DB (Advanced)", value=vis.get("species", False))

        if st.form_submit_button("Save settings"):
            STATE["settings"]["unique_sig"] = bool(uniq)
            STATE["settings"]["default_level"] = int(default_level)
            STATE["settings"]["hide_spinner"] = bool(hide_spinner)
            STATE["settings"]["visible_pages"] = {
                "pokedex": v_pokedex, "team": v_team, "matchup": v_matchup, "evo": v_evo,
                "opponents": v_opponents, "moves": v_moves, "species": v_species, "saveload": v_saveload
            }
            save_state(STATE); st.success("Settings saved. Sidebar updates on next rerun.")
...
st.sidebar.title("Firered/Leafgreen App")
